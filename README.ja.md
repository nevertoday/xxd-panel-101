<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 101 プロジェクトバナー" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 101｜九つの記憶ノート

### 一枚の写真を、覚えておきたい九つの生活の断片へ
...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 101 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

**Panel 101** は、一枚の写真にある気分、持ち物、小さな細部を、単なる写実的な描き直しではなく記憶として残したいときに適しています。同じ画面をゆるい九宮格の九つの記憶単位へ整理し、稚拙なドゥードル、クレヨン、色鉛筆の粒子で私的なビジュアル手帳の一ページにします。

### 向いている用途

- 細部の多い写真を、関連する九つの主体、動作、持ち物、感情の手がかりへ整理したい場合
- 写真のそのままの再現に、揺れる手描き線と乾性画材による私的な手触りを加えたい場合
- 隠れた3×3秩序を保ちながら、アイコンの大きさ、角度、位置をずらして余裕を作りたい場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 101 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 3×3記憶アイコン · 稚拙なドゥードル · loose grid · クレヨンと色鉛筆の粒子 · レトロキャンディーカラー · 手描き注釈
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 実写と九宮格の記憶リストをつなぐ、私的な手帳風の編集ポスター |
| **識別しやすい特徴** | ゆるい3×3アイコン、稚拙な手描き線、クレヨンと色鉛筆の粒子、暖かな生成り紙、少量の赤い注記 |
| **入力の尊重** | 九つの単位を、現在の写真に見える主体、細部、動作、持ち物、関係から作り、物語を勝手に置き換えない |
| **用途** | 旅の記録、暮らしの記念、家族写真、ペット日記、イベント回顧、SNS、複数端末壁紙 |
<!-- xxd-panel-benefit:end -->

## 変換ロジック

```text
写真の主体と生活の手がかりを読む → 記憶に残す九つの単位を選ぶ → 暗示的な3×3 loose grid に置く → 稚拙な線とクレヨン／色鉛筆で翻訳 → 元写真からレトロなキャンディーカラーを抽出 → 少量の手書き文字と赤い注釈を加える
```

## 完成作の特徴

- 九つのアイコンが写真の主体、特徴、持ち物、動作、感情、小場面を受け持ち、一つの記憶リストになります。
- グリッドの秩序は見えますが、大きさ、角度、位置を少しずらし、UI表のようにはしません。
- 線は揺れ、太さが不均一で閉じないこともあり、クレヨンと色鉛筆は紙目や粗い縁を残します。
- 暖かな生成り紙に低彩度の青緑、黄、ピンク、ごく少量の橙赤を置き、赤い円、矢印、星で小さく注記します。
- 文字は短い手書きタイトルや caption とし、写実、滑らかなベクター、重い影、商用テンプレート、児童カートゥーンを避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-101-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 101 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-101 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-101 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 101 を適用し、共通設定は一度だけ解決
/xxd-panel-101 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-101`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-101-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-101-prompt.zh-CN.md)
...
| **[xxd-panel-101](https://github.com/nevertoday/xxd-panel-101)** | 九つの記憶ノート · 3×3記憶アイコン · 稚拙なドゥードル · クレヨンと色鉛筆 · レトロ色 · 赤い手描き注釈 |
...
### Xiaoxiaodong 総控 · 将軍総指揮 Skill · CNY 101

CNY 101 の一回払いで、このシリーズの将軍総指揮 Skill（`xxd-panel-all`）を利用できます。全兵士 Skills の統括、推薦、指名派遣、一括調整に対応します。WeChat では「将軍総指揮 Skill」と記載してください。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–101） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
