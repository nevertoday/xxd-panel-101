...
# 🦁 XXD Panel 101｜Nine-Cell Memory Notebook

### Turn one photograph into nine small memories worth keeping
...
The eight works above comprise four 16:9 landscape left–right samples and four 3:4 portrait top–bottom samples. Every work was generated independently by Panel 101 from its own original brief; no artwork from another numbered Panel is reused. The samples use intelligent English copy.

<!-- xxd-human-intro:start -->
## What it solves

**Panel 101** is for photographs whose mood, objects, and small details deserve more than a literal redraw. It turns one image into nine related memory units in a loose grid, using naïve doodles, crayon and coloured-pencil grain to create a personal visual-notebook page.

### Best for

- organising a photograph rich in details into nine related subjects, actions, objects, and emotional cues;
- giving a literal photograph a more personal, hand-kept feeling through trembling lines and dry-media grain;
- balancing order and ease through an implicit 3×3 grid whose icons can shift in scale, angle, and position;
- top-bottom, left-right, design-only, multi-ratio, wallpaper, and directory-batch delivery;

### Quick start

> Use XXD Panel 101 on this image and recommend the most suitable composition and size first.
...
**Signature:** 3×3 memory icons · naïve doodles · loose grid · crayon and coloured-pencil grain · retro candy palette · handwritten annotations
...
|---|---|
| **What you get** | A private-notebook editorial poster connecting a real photograph to a nine-cell memory list |
| **Recognisable signature** | loose 3×3 icons, naïve hand lines, crayon and coloured-pencil grain, warm off-white paper, and sparse red notes |
...
```text
/xxd-panel-101 photo.jpg --mode top-bottom,design-only --size auto,3:4,9:16 --text prompt --locale ja-JP
```
...
```text
/xxd-panel-101 photo.jpg --prefs last
/xxd-panel-101 photo.jpg --prefs edit
/xxd-panel-101 photo.jpg --prefs new
```
...
/xxd-panel-101 photo.jpg --mode top-bottom --size auto --text prompt --locale en-GB

# Left-right comparison, 3:2 landscape, no text
/xxd-panel-101 photo.jpg --mode left-right --size 3:2 --text none

# Design-only, 9:16 phone portrait, Japanese text
/xxd-panel-101 photo.jpg --mode design-only --size 9:16 --text prompt --locale ja-JP

# Linked wallpaper pack: establish one anchor, then recompose every device
/xxd-panel-101 photo.jpg --mode wallpaper-pack --wallpaper linked --text none

# Independent wallpapers with exact device resolutions
/xxd-panel-101 photo.jpg --mode wallpaper-pack --wallpaper independent --wallpaper-size phone=1440x3200,ipad=2048x2732,desktop=3840x2160,watch=1024x1024 --text none

# Follow the source image's aspect ratio
/xxd-panel-101 photo.jpg --mode design-only --size source --text none

# Create square, portrait, and landscape compositions
/xxd-panel-101 photo.jpg --mode design-only --size 1:1,3:4,16:9 --text none

# Combine a custom ratio with exact pixel targets
/xxd-panel-101 photo.jpg --mode design-only --size 11:14,2160x3840,3840x2160 --text none

# Let the original prompt generate wording; constrain only the locale
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --text prompt --locale en-GB

# Use exact copy without rewriting, translating, or adding a title
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --copy "Let the light stay" --locale en-GB

# Two modes × three sizes produce six independently composed assets
/xxd-panel-101 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale en-GB

# Apply Panel 101 to a whole directory; shared settings resolve once
/xxd-panel-101 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale en-GB

# Repeated parameters accumulate; place the task under a chosen root
/xxd-panel-101 photo.jpg --mode top-bottom --mode left-right --size 3:4 --size 16:9 --text none --out ./deliveries
```
...
```bash
git clone https://github.com/nevertoday/xxd-panel-101.git
mkdir -p ~/.codex/skills
ln -s "$(pwd)/xxd-panel-101" ~/.codex/skills/xxd-panel-101
```

You can also install it directly with `npx skills`:

```bash
npx skills add https://github.com/nevertoday/xxd-panel-101 --skill xxd-panel-101
```
...
Claude Code users may link the same folder under `~/.claude/skills/xxd-panel-101`. Restart the agent session after installation.
...
Full specifications: [Skill workflow](SKILL.md) · [source archive](references/original-prompt/zh-CN.md) · [English runtime adapter](references/xxd-panel-101-prompt.en.md) · [Chinese runtime adapter](references/xxd-panel-101-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## Complete XXD Panel catalog

The current XXD Panel series runs from 001 through 112, and every Panel retains its own independent original prompt and aesthetic logic. The table below is the historical catalog as of this project’s release, listing 001 through 101; the current project is bold.
...
### Xiaoxiaodong Commander · General Command Skill · CNY 101

A one-time CNY 101 purchase unlocks this suite's General Command Skill (`xxd-panel-all`) for roster control, recommendations, Soldier dispatch, and batch coordination. Include “General Command Skill” in your WeChat message.
