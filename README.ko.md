<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 101 프로젝트 배너" width="1200"></p>

<div align="center">

# 🦁 XXD Panel 101｜아홉 칸 기억 노트

### 한 장의 사진을 간직하고 싶은 아홉 가지 생활의 기억으로 정리합니다
...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 101이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

**Panel 101**은 한 장의 사진에 담긴 분위기, 사물, 작은 디테일을 단순한 사실적 재현이 아닌 기억으로 남기고 싶을 때 적합합니다. 같은 장면을 느슨한 아홉 칸의 기억 단위로 정리하고, 서툰 손그림 낙서와 크레용·색연필 입자로 개인적인 비주얼 다이어리 한 페이지를 만듭니다.

### 이런 경우에 적합합니다

- 디테일이 많은 사진을 서로 연결된 아홉 가지 주체, 행동, 소지품, 감정의 단서로 정리하고 싶을 때
- 사진의 문자 그대로의 재현에 떨리는 손그림 선과 건식 재료의 사적인 온기를 더하고 싶을 때
- 암시적인 3×3 질서는 지키되 아이콘 크기, 각도, 위치를 어긋나게 해 여유를 만들고 싶을 때
- 상하·좌우·디자인 전용·여러 비율·벽지·폴더 일괄 처리

### 빠른 시작

> 이 이미지를 XXD Panel 101로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 3×3 기억 아이콘 · 서툰 낙서 · loose grid · 크레용과 색연필 입자 · 복고풍 캔디 팔레트 · 손글씨 주석
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 실제 사진과 아홉 칸 기억 목록을 연결하는 개인 다이어리형 편집 포스터 |
| **대표 특징** | 느슨한 3×3 아이콘, 서툰 손그림 선, 크레용·색연필 입자, 따뜻한 미색 종이, 소량의 빨간 메모 |
| **입력 존중 방식** | 아홉 단위를 현재 사진에서 식별되는 주체, 디테일, 행동, 사물, 관계로 만들며 이야기를 임의로 바꾸지 않음 |
| **활용처** | 여행 기록, 생활 기념, 가족 사진, 반려동물 일기, 행사 회고, 소셜 콘텐츠, 여러 기기 벽지 |
<!-- xxd-panel-benefit:end -->

## 변환 논리

```text
사진의 주체와 생활 단서 읽기 → 기억할 아홉 단위 선택 → 암시적인 3×3 loose grid에 배치 → 서툰 선과 크레용/색연필로 번역 → 원본에서 복고풍 캔디색 추출 → 소량의 손글씨와 빨간 주석 추가
```

## 완성작의 특징

- 아홉 아이콘이 사진의 주체, 특징, 사물, 행동, 감정, 작은 장면을 맡아 하나의 기억 목록이 됩니다.
- 그리드 질서는 느껴지지만 크기, 각도, 위치를 조금씩 어긋나게 해 UI 표처럼 보이지 않습니다.
- 선은 떨리고 굵기가 일정하지 않으며 열려 있을 수 있고, 크레용과 색연필은 종이결과 거친 가장자리를 남깁니다.
- 따뜻한 미색 종이 위에 저채도 청록, 노랑, 분홍, 극소량의 주황빛 빨강을 놓고 빨간 원·화살표·별을 메모처럼 씁니다.
- 글자는 짧은 손글씨 제목이나 caption처럼 쓰며, 사실주의, 매끈한 벡터, 무거운 그림자, 상업 템플릿, 아동 카툰을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-101-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 101의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-101 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-101 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-101 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-101 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 101 적용, 공통 설정은 한 번만 확정
/xxd-panel-101 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-101`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-101-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-101-prompt.zh-CN.md)
...
| **[xxd-panel-101](https://github.com/nevertoday/xxd-panel-101)** | 아홉 칸 기억 노트 · 3×3 기억 아이콘 · 서툰 낙서 · 크레용과 색연필 · 복고색 · 빨간 손글씨 주석 |
...
### Xiaoxiaodong 총괄 · 장군 총지휘 Skill · CNY 101

CNY 101을 한 번 결제하면 이 시리즈의 장군 총지휘 Skill(`xxd-panel-all`)을 사용할 수 있습니다. 전체 병사 Skills의 통합 관리, 추천, 지정 파견, 일괄 조정을 지원합니다. WeChat 메시지에 “장군 총지휘 Skill”이라고 적어 주세요.
...
| **병사급** | `xxd-panel-NNN` (현재 001–101) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
