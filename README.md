...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 101 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。

<!-- xxd-human-intro:start -->
## 适用场景与解决的问题

**Panel 101** 适合那些想把一张照片的情绪、物件和小细节保存下来，却不想得到普通写实复刻的人。它把同一画面提炼成松散九宫格中的九个记忆单元，以手绘稚拙涂鸦、蜡笔和彩铅颗粒，形成一页有私人感的视觉记忆簿。

### 适合这些情况

- **一张照片里值得记住的细节太多：** 把主体、动作、随身物件和情绪符号整理成九个彼此相关的记忆单元。
- **普通照片复刻缺少私人感：** 用略带抖动的手绘线、蜡笔与彩铅颗粒，让画面像真实写下的手帐页。
- **网格容易太规整或太杂乱：** 以隐性 3×3 秩序控制版面，同时允许图标轻微错位，保留松弛与呼吸感。
...
> 用 XXD Panel 101 帮我处理这张图，先推荐最合适的构图和尺寸。
...
**关键词：** 3×3 memory icons · naïve doodles · loose grid · crayon and coloured-pencil grain · retro candy palette · handwritten annotations

<!-- xxd-panel-benefit:start -->
## 快速判断：XXD Panel 101 适合你吗？

| 你关心的问题 | 这套风格给你的回答 |
|---|---|
| **你会得到什么** | 一张把现实照片与九宫格记忆清单连接起来的私人手帐式编辑海报 |
| **一眼可辨的特点** | 松散 3×3 图标、稚拙手绘线、蜡笔和彩铅颗粒、暖米白纸、少量红色批注 |
| **它如何尊重你的输入** | 九个单元都来自当前照片可辨认的主体、局部、动作、物件或叙事关系，不凭空替换故事 |
| **可以用在哪里** | 旅行记录、生活纪念、家庭影像、宠物日记、活动回顾、社交内容与多端壁纸 |
<!-- xxd-panel-benefit:end -->

## 它如何把你的输入变成独特成品

```text
读取照片中的主体与生活线索 → 选出九个最值得记住的单元 → 放入隐性 3×3 松散网格 → 用稚拙手绘线与蜡笔／彩铅转译 → 从原图提炼复古糖果色 → 加入少量手写文字与红色批注
```

## 成品中最容易识别的风格特点

- 九个图标分别承接当前照片的主体、特征、物件、动作、情绪或小场景，形成同一张照片的记忆清单。
- 网格秩序隐约可见，但图标的大小、角度与落点轻微错开，不像 UI 图标表格。
- 线条略抖、粗细不一且不完全闭合；蜡笔和彩铅不过度填满，保留纸纹、白色颗粒和毛糙边缘。
- 暖米白纸面承载低饱和蓝绿、黄粉与极少量橘红，并用少量红色圈、箭头或星点作私人批注。
- 文字像随手记录的短标题与 caption，少而有意义；拒绝写实插画、光滑矢量、重阴影、商业模板和儿童卡通化。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-101-prompt.en.md)
...
直接传入一个图片目录时，这个士兵 Skill 会自动批量处理目录中的图片，并让每张图片都使用 Panel 101 的同一套原始审美。默认递归扫描常见位图，先报告数量，再统一确认一次模式、尺寸、文字和语言；每张图片独立生成，互不借用内容或文案。整批仍然只创建一个扁平任务目录，单张失败会被记录，但不会静默跳过后续图片。
...
/xxd-panel-101 photo.jpg --mode top-bottom --size auto --text prompt --locale zh-CN
...
# 使用准确文案，不改写、不翻译、不补标题
/xxd-panel-101 photo.jpg --mode design-only --size 3:4 --copy "让风经过这里" --locale zh-CN

# 两种模式 × 三种尺寸，共生成六张独立完整构图
/xxd-panel-101 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale zh-CN

# 整个目录统一使用 Panel 101；公共设置只解析一次
/xxd-panel-101 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale zh-CN
...
Claude Code 用户可把同一文件夹链接到 `~/.claude/skills/xxd-panel-101`. 安装后请重启 Agent 会话。
...
完整规格: [Skill 工作流](SKILL.md) · [原始风格档案](references/original-prompt/zh-CN.md) · [英文运行适配器](references/xxd-panel-101-prompt.en.md) · [中文运行适配器](references/xxd-panel-101-prompt.zh-CN.md)
...
| **[xxd-panel-101](https://github.com/nevertoday/xxd-panel-101)** | 九宫记忆簿 · 3×3 记忆图标 · 稚拙涂鸦 · 蜡笔彩铅颗粒 · 复古糖果色 · 手写红色批注 |
